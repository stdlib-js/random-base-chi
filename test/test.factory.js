...
tape( 'the function returns a seeded pseudorandom number generator (integer seed)', function test( t ) {
	...
	t.notEqual( chi1, chi2, 'separate generators' );
...
tape( 'attached to the returned function is the generator name', function test( t ) {
	var chi = factory();
	t.strictEqual( chi.NAME, 'chi', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var chi = factory();
	t.strictEqual( typeof chi.PRNG, 'function', 'has property' );

	chi = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chi.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( chi.seed ), true, 'has property' );
	t.strictEqual( chi.seed[ 0 ], 12345, 'equal to provided seed' );

	chi = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( chi.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var chi = factory();
	t.strictEqual( typeof chi.seedLength, 'number', 'has property' );

	chi = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chi.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var chi = factory();
	t.strictEqual( isUint32Array( chi.state ), true, 'has property' );

	chi = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chi.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var chi = factory();
	t.strictEqual( typeof chi.stateLength, 'number', 'has property' );

	chi = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chi.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var chi = factory();
	t.strictEqual( typeof chi.byteLength, 'number', 'has property' );

	chi = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chi.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	chi = factory();
	t.strictEqual( typeof chi.toJSON, 'function', 'has method' );

	o = chi.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, chi.NAME, 'has property' );
	t.deepEqual( o.state, typedarray2json( chi.state ), 'has property' );
	...
	t.strictEqual( typeof chi.toJSON, 'function', 'has method' );
	t.strictEqual( chi.toJSON(), null, 'returns expected value' );
...
tape( 'the function returns a PRNG for generating random numbers from a chi distribution', opts, function test( t ) {
	var threshold;
	var count;
	var npass;
	var N;
	...
	function done( bool, rejected ) {
		t.ok( bool, 'null hypothesis (i.e., that numbers are drawn from chi('+k+') is rejected in less than '+(threshold*100)+'% of cases ('+rejected+' of '+N+'). Repeats: '+npass+' of '+count+'.' );
		t.end();
	}
});
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( chi(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
